...
因为 merge 是 opaque 的,责任也在你这边：你设的 key 会覆盖 adapter 算出来的值,格式错误的值会以 provider 端的 `InvalidRequest` 浮现,而不是编译错误。这是有意的取舍——直达 provider 特性、不必等核心类型加字段。

`Agent::builder(...)` 上挂了同一个 setter——一次配好,工具循环的每一步都会带着它发出去：
...
有些 provider 特性是挂在**某条消息**或**某个内容块**上的——Anthropic 的 `cache_control` 断点就是典型：它告诉 API 从哪个 block 开始 cache，"挂在哪里"本身就是语义。同样的 `provider_options` 形状在 message 和 block 层级都能用：
...
`Message::with_provider_options(...)` 是 message 级对应物，merge 进 message 对象本身而不是它里面的某个 block。

opaque 契约在每一层都成立——adapter 只读自己的 slug、merge 它找到的内容、从不发明语义。OpenAI Responses API 和 openai-compatible Chat Completions 默认保留 plain string 的 `content` 形态；只有当某个 text block 带 `provider_options`（或者出现 image）时,它们才切到 typed-array 形态——bare string 装不下 per-block 字段。

| 调用点                                                | merge 进                                                |
| ----------------------------------------------------- | ------------------------------------------------------- |
| `GenerateTextRequest::builder().provider_options(…)`  | 请求体顶层                                              |
| `Agent::builder().provider_options(…)`                | 每一步请求体的顶层                                      |
