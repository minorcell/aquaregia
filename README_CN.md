...
- **Axum SSE 桥接** —— `axum_sse::stream_to_sse(stream)` 一行将任意 `TextStream` 转成命名 SSE 事件。
...
| `axum`      | `axum_sse` 模块：将 `TextStream` 转为 Axum SSE 响应                |
...
启用 `axum` feature 后，可以把任意 `TextStream` 直接接到 `Sse` 响应里：
...
桥接器把每个 `StreamEvent` 映射成命名 SSE 事件，浏览器端可按事件名分流：
