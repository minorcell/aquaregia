...
Some providers run native tools server-side. Aquaregia does not execute those tools; the provider handles them within the model call.
...
Top-level provider option keys overwrite the payload keys Aquaregia already computed. This matters for `tools`: if you pass `provider_options.<slug>.tools`, it replaces Aquaregia's generated `tools` array for that provider request. Prefer normal Aquaregia tools for Rust executors, and use provider-native `tools` only when the provider runs them itself.
