...
`user(...)` and `system(...)` are convenience setters for one-message requests. Use `message(...)` when building a real conversation.
...
| `user(prompt)` | replace messages with one user message |
| `system(instruction)` | replace messages with one system message |
